...
complete_line(FirstC, special, [A|Atomics]):- 	% special char
    ...
    name(A, [FirstC]),
    read_atomics(Atomics).
complete_line(FirstC, ialpha,[A|[]]):-	% begin word
    !,
    name(A, [FirstC]), !.
    %read_atomics(Atomics).
...
complete_word(FirstC,FirstT,[],FirstC,FirstT). % where FirstT is not alpha
...
char_type(46,ialpha,46):-!.  % '.'
char_type(63,ialpha,63):-!.  % '?'
